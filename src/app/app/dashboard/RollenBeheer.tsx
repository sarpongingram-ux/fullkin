...
export function RollenBeheer({ rollen, leden }: { rollen: Rol[]; leden: Lid[] }) {
  ...
  return (
    <section className="fk-card">
      <h2 className="font-black text-inkt text-lg mb-1">De rolstructuur</h2>
      ...
      <ul className="space-y-2">
        {rollen.map((r) => (
          <li
            key={r.role}
            className={`rounded-2xl bg-white p-4 ${r.ontgrendeld ? "" : "opacity-60"}`}
            style={{ boxShadow: "var(--schaduw)" }}
          >
            <div className="flex items-center justify-between gap-3">
              <div className="min-w-0">
                <p className="font-bold text-inkt">{rolNamen[r.role]}</p>
              ...
              {r.role === "co_founder" ? (
                <span className="text-sm text-goud font-bold shrink-0">
                  permanent
                </span>
              ) : r.ontgrendeld ? (
                <select
                  ...
                  className="text-sm rounded-xl border-2 border-rand bg-white px-3 py-2 text-inkt outline-none focus:border-terracotta shrink-0 max-w-[45%]"
                >
                  <option value="">
